...
test.describe('All Workflow Tests', () => {
    ...
    test('[@regression] Navigate to Register1', async ({ page }) => {
...
const register= new RegisterPO(page);
    await register.UserInfo(inputData.firstName, inputData.lastName, inputData.country);
    await register.Credentials(inputData.userName, inputData.Email, inputData.Password);
    ...
    test('[@smoke] Password Validation',async({page})=>{
     ...
     await register.password.locator('..').locator('.password-input-toggle').click({timeout: 5000});
